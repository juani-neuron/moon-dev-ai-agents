...
class GannConvergence(Strategy):
    ...
    def calculate_position_size(self, entry_price, stop_price):
        ...
        # Convert to integer for backtesting.py - CRITICAL FIX 🌙
        ...
        print(f"💰 Moon Dev Position Size Calculation:")
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (current_trend == 1 and  # Uptrend
                ...
                if position_size > 0:
                    ...
                    # Ensure TP > entry for longs (minimum 2:1 R:R fallback)
                    risk = price - stop_price
                    if target_price <= price:
                        target_price = price + (2 * risk)
                    ...
                    # Enter position with integer size
            ...
            # SHORT ENTRY
                ...
                if position_size > 0:
                    ...
                    # Ensure TP < entry for shorts (minimum 2:1 R:R fallback)
        ...
        # EXIT LOGIC for existing positions — SL/TP managed by orders, no trailing stop override
        else:
            pass

# Data preparation function
def prepare_data(filepath):
    print(f"📊 Moon Dev Loading data from: {filepath}")
    ...
    for req in required_cols:
        if req.capitalize() not in data.columns:
            print(f"⚠️  Moon Dev Warning: Missing required column: {req.capitalize()}")
    ...
    print(f"✅ Moon Dev Data loaded: {len(data)} rows, {len(data.columns)} columns")
...
if __name__ == "__main__":
    ...
    if 'Open' not in data.columns or 'Close' not in data.columns:
        print("❌ Moon Dev Error: Required price columns not found in data")
        exit()
    ...
    # Print results
    print("\n" + "=" * 50)
    print("📈 MOON DEV BACKTEST RESULTS - GANNCONVERGENCE STRATEGY")
    print("=" * 50)
    print(stats)
    print("\n" + "=" * 50)
