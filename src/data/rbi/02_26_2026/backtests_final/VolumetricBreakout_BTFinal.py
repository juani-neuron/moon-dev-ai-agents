...
class VolumetricBreakout(Strategy):
    ...
    def next(self):
        ...
        # Check if we're in a trade — sl/tp are set at entry, just check volume exit
        if self.position:
            if len(self.volume_sma) > 3 and current_volume < self.volume_sma[-1] * 0.5:
                self.position.close()
            return
                ...
                if long_condition:
                    ...
                    if risk_per_share > 0:
                        ...
                        if position_size > 0:
                            ...
                            tp = current_price + risk_per_share * self.reward_ratio
