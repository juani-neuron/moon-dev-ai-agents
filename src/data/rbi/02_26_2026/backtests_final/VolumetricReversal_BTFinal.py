...
class VolumetricReversal(Strategy):
    ...
    def init(self):
        ...
        self.vwrsi = self.I(self._calc_vwrsi, self.data.Close, self.data.Volume,
                            self.vwrsi_period, self.volume_sma_period)
    ...
    @staticmethod
    def _calc_vwrsi(close, volume, rsi_period, vol_sma_period):
        rsi = talib.RSI(close, timeperiod=rsi_period)
        sma_vol = talib.SMA(volume, timeperiod=vol_sma_period)
        sma_vol = np.where((sma_vol == 0) | np.isnan(sma_vol), 1, sma_vol)
        return (rsi * volume) / sma_vol
            ...
            if current_bar - self.oversold_candle_index <= 20:
                ...
                if breakout_condition and volume_condition:
                    ...
                    if risk_per_share > 0:
                        ...
                        # Enter long position with fractional size
