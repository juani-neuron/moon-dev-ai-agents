...
class VolatilityBreakout(Strategy):
    ...
    def init(self):
        ...
        self.bb_mid = self.I(talib.SMA, self.data.Close, timeperiod=self.bb_period, name='BB_MID')
        self.bb_upper = self.I(lambda close: self.bb_mid + (2 * talib.STDDEV(close, timeperiod=self.bb_period)), 
                              self.data.Close, name='BB_UPPER')
        self.bb_lower = self.I(lambda close: self.bb_mid - (2 * talib.STDDEV(close, timeperiod=self.bb_period)), 
        ...
        self.bbw_sma = self.I(talib.SMA, bb_width, timeperiod=self.bbw_sma_period, name='BBW_SMA')
        ...
        self.prev_close = self.I(lambda: pd.Series(self.data.Close).shift(1), name='PREV_CLOSE')
        
        print("🌙 Moon Dev's Volatility Breakout Strategy Initialized!")
        print(f"✨ BB Period: {self.bb_period}, BB Std: {self.bb_std}")
        print(f"✨ BBW SMA Period: {self.bbw_sma_period}")
    ...
    def next(self):
        ...
        if not self.position:
            ...
            breakout_condition = self.prev_close[-1] < self.bb_lower[-1]
            ...
            # Check stop loss (handled automatically by backtesting.py)
